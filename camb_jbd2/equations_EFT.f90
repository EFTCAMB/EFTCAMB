    ...
    !MODIFIED BY NELSON
    if (CP%EFTFlag .eq. 2 .and. CP%DesignerEFTmodel .eq. 4) then
    ...
    !Modified by Nelson !Dummy routine to extract the correct H0 value for JBD cosmologies
	...
	!Modified by Nelson! Need to call brans-dicke models from the beginning to have them in dtauda
    	! EFTCAMB MOD START
    	!NECESSARY CALL TO RUN CAMB ON ITS OWN AND WITH COSMOMC
    	if ((EFTCAMBuseinCOSMOMC==1.or.EFTCAMBuseinCOSMOMC==0).and.CP%EFTflag==2.and.CP%DesignerEFTmodel==4) then
    	call EFTCAMB_initialization(EFTsuccess)
    	!if (.not.EFTsuccess) stop !bh
	!Modified by Nelson
    	end if
        ! EFTCAMB MOD END.
   
        !MODIFIED BY NELSON
	if (CP%EFTflag == 2.and.CP%DesignerEFTmodel==4)then
	...
	!MODIFIED BY NELSON
    	if (CP%DesignerEFTmodel .eq. 4) then
	...
	!MODIFIED BY NELSON
	    ...
	    !Modified by Nelson
            if (CP%EFTflag==2.and.(CP%DesignerEFTmodel==1 .or. CP%DesignerEFTmodel == 4)) then	
	    !Modified by Nelson
                EFTc = EFTcTemp(a,0)
