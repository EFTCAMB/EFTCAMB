    ...
    !Modified by Nelson
    subroutine CAMBParams_Set(P, Haux, error, DoReion)
    !Modified by Nelson
    use constants
    type(CAMBparams), intent(in) :: P
    !Modified by Nelson!
    real(dl), intent(out) :: Haux
    !Modified by Nelson
...
!Modified by Nelson
	...
	!MODIFIED BY NELSON (NEED TO RECOVER H0 TODAY FROM BRANS-DICKE EVOLUTION. THIS IS THE PERFECT TIME!!!
            call init_background(Haux)
      		    if (CP%EFTFlag .eq. 2 .and. CP%DesignerEFTmodel .eq. 4) then
                    CP%H0=Haux     
                    Haux=H0
		    end if
