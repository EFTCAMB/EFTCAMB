...
contains
    ...
    subroutine EFTCAMBPureEFTstdAllocateModelSelection( self )
        ...
        class(EFTCAMB_std_pure_EFT)                       :: self              !< the base class
        character(len=EFT_names_max_length), allocatable, dimension(:)              :: param_names       !< an array of strings containing the names of the function parameters
        ...
        select case ( self%PureEFTmodelOmega )
            ...
            case(3)
                ...
                allocate( character(len=EFT_names_max_length):: self%PureEFTOmega%param_names(self%PureEFTOmega%parameter_number) )
                allocate( character(len=EFT_names_latex_max_length):: self%PureEFTOmega%param_names_latex(self%PureEFTOmega%parameter_number) )
                allocate( param_names(2)       )
                allocate( param_names_latex(2) )
                param_names(1) = TRIM('EFTOmega0')
                param_names(2) = TRIM('EFTOmegaExp')
                param_names_latex(1) = TRIM('\f$ \Omega_0 \f$')
                param_names_latex(2) = TRIM('\f$ \Omega_{\text{Exp}} \f$')
                call self%PureEFTOmega%set_param_names(param_names, param_names_latex)
        ...
        select case ( self%EFTwDE )
            ...
            case(2)
                ...
                allocate( character(len=EFT_names_max_length):: self%PureEFTwDE%param_names(self%PureEFTwDE%parameter_number) )
                allocate( character(len=EFT_names_latex_max_length):: self%PureEFTwDE%param_names_latex(self%PureEFTwDE%parameter_number) )
                allocate( param_names(2)       )
                allocate( param_names_latex(2) )
            ...
            case(5)
                ...
                allocate( character(len=EFT_names_max_length):: self%PureEFTwDE%param_names(self%PureEFTwDE%parameter_number) )
                allocate( character(len=EFT_names_latex_max_length):: self%PureEFTwDE%param_names_latex(self%PureEFTwDE%parameter_number) )
                allocate( param_names(4)       )
                allocate( param_names_latex(4) )
                param_names(1) = TRIM('EFTw0')
                param_names(2) = TRIM('EFTwa')
        ...
        select case ( self%PureEFTmodelGamma1 )
            ...
            case(3)
                ...
                allocate( character(len=EFT_names_max_length):: self%PureEFTGamma1%param_names(self%PureEFTGamma1%parameter_number) )
                allocate( character(len=EFT_names_latex_max_length):: self%PureEFTGamma1%param_names_latex(self%PureEFTGamma1%parameter_number) )
                allocate( param_names(2)       )
                allocate( param_names_latex(2) )
                param_names(1) = TRIM('EFTGamma10'  )
                param_names(2) = TRIM('EFTGamma1Exp')
                param_names_latex(1) = TRIM('\f$ \gamma_1^0 \f$')
                param_names_latex(2) = TRIM('\f$ \gamma_1^{\text{Exp}} \f$')
                call self%PureEFTGamma1%set_param_names(param_names, param_names_latex)
                deallocate(param_names)
                deallocate(param_names_latex)
            case(4)
        ...
        select case ( self%PureEFTmodelGamma2 )
            ...
            case(3)
                ...
                allocate( character(len=EFT_names_max_length):: self%PureEFTGamma2%param_names(self%PureEFTGamma2%parameter_number) )
                allocate( character(len=EFT_names_latex_max_length):: self%PureEFTGamma2%param_names_latex(self%PureEFTGamma2%parameter_number) )
                allocate( param_names(2)       )
                allocate( param_names_latex(2) )
                param_names(1) = TRIM('EFTGamma20'  )
                param_names(2) = TRIM('EFTGamma2Exp')
                param_names_latex(1) = TRIM('\f$ \gamma_2^0 \f$')
                param_names_latex(2) = TRIM('\f$ \gamma_2^{\text{Exp}} \f$')
                call self%PureEFTGamma2%set_param_names(param_names, param_names_latex)
                deallocate(param_names)
                deallocate(param_names_latex)
            case(4)
        ...
        select case ( self%PureEFTmodelGamma3 )
            ...
            case(3)
                ...
                allocate( character(len=EFT_names_max_length):: self%PureEFTGamma3%param_names(self%PureEFTGamma3%parameter_number) )
                allocate( character(len=EFT_names_latex_max_length):: self%PureEFTGamma3%param_names_latex(self%PureEFTGamma3%parameter_number) )
                allocate( param_names(2)       )
                allocate( param_names_latex(2) )
                param_names(1) = TRIM('EFTGamma30  ')
                param_names(2) = TRIM('EFTGamma3Exp')
                param_names_latex(1) = TRIM('\f$ \gamma_3^0 \f$')
                param_names_latex(2) = TRIM('\f$ \gamma_3^{\text{Exp}} \f$')
                call self%PureEFTGamma3%set_param_names(param_names, param_names_latex)
                deallocate(param_names)
                deallocate(param_names_latex)
            case(4)
        ...
        if ( .not. self%PureEFTHorndeski ) then
            ...
            select case ( self%PureEFTmodelGamma4 )
                ...
                case(3)
                    ...
                    allocate( character(len=EFT_names_max_length):: self%PureEFTGamma4%param_names(self%PureEFTGamma4%parameter_number) )
                    allocate( character(len=EFT_names_latex_max_length):: self%PureEFTGamma4%param_names_latex(self%PureEFTGamma4%parameter_number) )
                    allocate( param_names(2)       )
                    allocate( param_names_latex(2) )
                    param_names(1) = TRIM('EFTGamma40'  )
                    param_names(2) = TRIM('EFTGamma4Exp')
                    param_names_latex(1) = TRIM('\f$ \gamma_4^0 \f$')
                    param_names_latex(2) = TRIM('\f$ \gamma_4^{\text{Exp}} \f$')
                    call self%PureEFTGamma4%set_param_names(param_names, param_names_latex)
                    deallocate(param_names)
                    deallocate(param_names_latex)
                case(4)
            ...
            select case ( self%PureEFTmodelGamma5 )
                ...
                case(3)
                    ...
                    allocate( character(len=EFT_names_max_length):: self%PureEFTGamma5%param_names(self%PureEFTGamma5%parameter_number) )
                    allocate( character(len=EFT_names_latex_max_length):: self%PureEFTGamma5%param_names_latex(self%PureEFTGamma5%parameter_number) )
                    allocate( param_names(2)       )
                    allocate( param_names_latex(2) )
                    param_names(1) = TRIM('EFTGamma50'  )
                    param_names(2) = TRIM('EFTGamma5Exp')
                    param_names_latex(1) = TRIM('\f$ \gamma_5^0 \f$')
                    param_names_latex(2) = TRIM('\f$ \gamma_5^{\text{Exp}} \f$')
                    call self%PureEFTGamma5%set_param_names(param_names, param_names_latex)
                    deallocate(param_names)
                    deallocate(param_names_latex)
                case(4)
            ...
            select case ( self%PureEFTmodelGamma6 )
                ...
                case(3)
                    ...
                    allocate( character(len=EFT_names_max_length):: self%PureEFTGamma6%param_names(self%PureEFTGamma6%parameter_number) )
                    allocate( character(len=EFT_names_latex_max_length):: self%PureEFTGamma6%param_names_latex(self%PureEFTGamma6%parameter_number) )
                    allocate( param_names(2)       )
                    allocate( param_names_latex(2) )
                    param_names(1) = TRIM('EFTGamma60'  )
                    param_names(2) = TRIM('EFTGamma6Exp')
                    param_names_latex(1) = TRIM('\f$ \gamma_6^0 \f$')
                    param_names_latex(2) = TRIM('\f$ \gamma_6^{\text{Exp}} \f$')
                    call self%PureEFTGamma6%set_param_names(param_names, param_names_latex)
                    deallocate(param_names)
                    deallocate(param_names_latex)
                case(4)
