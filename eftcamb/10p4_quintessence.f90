...
contains
    ...
    subroutine EFTCAMB5eReadModelSelectionFromFile( self, Ini )
    ...
    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that reads the parameters of the model from file.
    subroutine EFTCAMB5eInitModelParametersFromFile( self, Ini )
        ...
        if ( .not. self%drag_initial_conditions ) then
            self%phidot_ini    = Ini_Read_Double_File( Ini, 'phidot_ini', 0._dl )
        end if
    ...
    subroutine EFTCAMB5eInitBackground( self, params_cache, feedback_level, success, outroot )
        ...
        if ( DebugEFTCAMB ) then
            call CreateTxtFile( TRIM(outroot)//'background_5e.dat', 32 )
            ...
            do Debug_n = 1, Debug_MaxNum
                ...
                write(32,'(20ES11.2E3)') debug_phi, self%quintessence_potential(debug_phi,0), H02, params_cache%h0_Mpc**2
            end do
            close(32)
        end if
        ...
        if ( DebugEFTCAMB ) then
            ...
            call CreateTxtFile( TRIM(outroot)//'background_5e_solution_1.dat', 33 )
            call CreateTxtFile( TRIM(outroot)//'background_5e_solution_2.dat', 34 )
            call CreateTxtFile( TRIM(outroot)//'background_5e_solution_3.dat', 35 )
            write (33,'(a)')         '# x a z phi phi_prime phi_prime_prime H2 DHoH Hdot grho_matter gpres_matter'
            write (34,'(a)')         '# x a z Ek_phi Ep_phi DEk_DN DEp_DN rho_phi pres_phi Drho_phi_DN Dpres_phi_DN energy_conservation w_phi lambda'
            write (35,'(a)')         '# x a z omega_r_t omega_m_t omega_nu_t omega_phi_t omega_tot_t'
    ...
    contains
        ...
        subroutine output( num_eq, ind, x, y )
            ...
            if ( DebugEFTCAMB ) then
                ...
                if ( is_open ) then
                    write (33,'(200ES15.4E3)') x, a, 1._dl/a-1._dl, phi, phi_prime, phi_prime_prime, H2, ( H2-( grho_matter +params_cache%grhov*a2 )/3._dl)/H2, Hdot, grho_matter, gpres_matter
                end if
                inquire( unit=34, opened=is_open )
                if ( is_open ) then
                    write (34,'(200ES15.4E3)') x, a, 1._dl/a-1._dl, Ek_phi, Ep_phi, DEk_DN, DEp_DN, rho_phi, pres_phi, Drho_phi_DN, Dpres_phi_DN, energy_conservation, w_phi, lambda
                end if
                inquire( unit=35, opened=is_open )
                if ( is_open ) then
                    write (35,'(200ES15.4E3)') x, a, 1._dl/a-1._dl, omega_r_t, omega_m_t, omega_nu_t, omega_phi_t, omega_tot_t
                end if
            end if
