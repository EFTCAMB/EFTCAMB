...
module EFTCAMB_full_Quartic_Galileon
    ...
    public EFTCAMB_Quartic_Galileon

    !----------------------------------------------------------------------------------------
    !> This is the type that contains the definition of the Quartic Galileon model.
    ...
    contains
        ...
        procedure :: read_model_selection            => EFTCAMBQuarticGalileonReadModelSelectionFromFile  !< subroutine that reads the parameters of the model from file
        procedure :: allocate_model_selection        => EFTCAMBQuarticGalileonAllocateModelSelection      !< subroutine that allocates the model selection. For Horava this is a dummy procedure.
        procedure :: init_model_parameters           => EFTCAMBQuarticGalileonInitModelParameters         !< subroutine that initializes the model parameters based on the values found in an input array.
        procedure :: init_model_parameters_from_file => EFTCAMBQuarticGalileonInitModelParametersFromFile !< subroutine that reads the parameters of the model from file.

        ! background solver:
        procedure :: initialize_background           => EFTCAMBQuarticGalileonInitBackground               !< subroutine that initializes the background of Quartic Galileon.
        procedure :: solve_background                => EFTCAMBQuarticGalileonSolveBackground            !< subroutine that solves the background equations.

        ! utility functions:
        procedure :: compute_param_number  => EFTCAMBQuarticGalileonComputeParametersNumber    !< subroutine that computes the number of parameters of the model.
        procedure :: feedback              => EFTCAMBQuarticGalileonFeedback                   !< subroutine that prints on the screen feedback information about the model.
        procedure :: parameter_names       => EFTCAMBQuarticGalileonParameterNames             !< subroutine that returns the i-th parameter name of the model.
        procedure :: parameter_names_latex => EFTCAMBQuarticGalileonParameterNamesLatex        !< subroutine that returns the i-th parameter name of the model.
        procedure :: parameter_values      => EFTCAMBQuarticGalileonParameterValues            !< subroutine that returns the i-th parameter value.

        ! CAMB related procedures:
        procedure :: compute_background_EFT_functions  => EFTCAMBQuarticGalileonBackgroundEFTFunctions   !< subroutine that computes the value of the background EFT functions at a given time.
        procedure :: compute_secondorder_EFT_functions => EFTCAMBQuarticGalileonSecondOrderEFTFunctions  !< subroutine that computes the value of the second order EFT functions at a given time.
        procedure :: compute_adotoa                    => EFTCAMBQuarticGalileonComputeAdotoa            !< subroutine that computes adotoa = H and its two derivatives wrt conformal time.
        procedure :: compute_H_derivs                  => EFTCAMBQuarticGalileonComputeHubbleDer         !< subroutine that computes the two derivatives wrt conformal time of H.

        ! stability procedures:
        procedure :: additional_model_stability        => EFTCAMBQuarticGalileonAdditionalModelStability !< function that computes model specific stability requirements.

    end type EFTCAMB_Quartic_Galileon
...
contains
    ...
    subroutine EFTCAMBQuarticGalileonReadModelSelectionFromFile( self, Ini )
    ...
    end subroutine EFTCAMBQuarticGalileonReadModelSelectionFromFile

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that allocates the model selection. Nothing needs to be done
    !! but procedure present because it is deferred.
    subroutine EFTCAMBQuarticGalileonAllocateModelSelection( self )

        implicit none

        class(EFTCAMB_Quartic_Galileon) :: self !< the base class

    end subroutine EFTCAMBQuarticGalileonAllocateModelSelection
    ...
    subroutine EFTCAMBQuarticGalileonInitModelParameters( self, array )
    ...
    end subroutine EFTCAMBQuarticGalileonInitModelParameters

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that reads the parameters of the model from file. Nothing needs to be done
    !! but procedure present because it is deferred.
    subroutine EFTCAMBQuarticGalileonInitModelParametersFromFile( self, Ini )
    ...
    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that initializes the background of Quartic Galileon.
    subroutine EFTCAMBQuarticGalileonInitBackground( self, params_cache, feedback_level, success )
    ...
    end subroutine EFTCAMBQuarticGalileonInitBackground

    subroutine EFTCAMBQuarticGalileonSolveBackground( self, params_cache, success )
      ...
      subroutine output( eft_par_cache, ind, x)
        ...
        Hdddd = 14._dl*adotoa*Hddd -71._dl*adotoa**2*Hdotdot +Hdotdot**2/adotoa +3._dl*Hdot*Hddd/adotoa&
         ...
         ! compute the background EFT functions:
          self%EFTOmega%y(ind)    =-(adotoa**4*phip1**4*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) &
              &+6*self%QuarticGalileon_c5*(Hdot*phip1 + a*adotoa**2*phip2)))/(2.*a*eft_par_cache%h0_Mpc**6*m0**5)
          !
          self%EFTOmega%yp(ind)    = -((adotoa**2*phip1**3*(-2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(Hdot*phip1 &
                    &+ a*adotoa**2*phip2) +3*self%QuarticGalileon_c5*(4*Hdot**2*phip1**2 + adotoa*phip1**2*Hdotdot &
                    &-adotoa**2*Hdot*phip1*(phip1 - 11*a*phip2) +a**2*adotoa**4*(4*phip2**2 + phip1*phip3))))/(a**2*eft_par_cache%h0_Mpc**6*m0**5))
                    !
          self%EFTOmega%ypp(ind)   = -((phip1**2*(4*Hdot**2*phip1**2*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) &
                    &+ 6*self%QuarticGalileon_c5*Hdot*phip1) +adotoa*phip1**2*(-2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 &
                    &+33*self%QuarticGalileon_c5*Hdot*phip1)*Hdotdot +3*adotoa**3*self%QuarticGalileon_c5*phip1**2*Hdotdot*(-3*phip1 + 16*a*phip2) &
                    &+adotoa**2*phip1*(2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(phip1 - 8*a*phip2) &
                    &+3*self%QuarticGalileon_c5*phip1*(Hdot**2*(-12*phip1 + 64*a*phip2) +phip1*Hddd)) +adotoa**4*(&
                    &-2*a**3*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(3*phip2**2 + phip1*phip3) +3*self%QuarticGalileon_c5*Hdot*phip1*(2*phip1**2 &
                    &+ 68*a**2*phip2**2 +a*phip1*(-16*phip2 + 17*a*phip3))) +3*a**3*adotoa**6*self%QuarticGalileon_c5*(12*phip2**3 &
                    &+ 12*phip1*phip2*phip3 +phip1**2*phip4)))/(a**3*eft_par_cache%h0_Mpc**6*m0**5))
                    !
          self%EFTOmega%yppp(ind)  = -((phip1*(5*Hdot*phip1**3*(-2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 &
                    &+ 21*self%QuarticGalileon_c5*Hdot*phip1)*Hdotdot + adotoa*phip1**2*(-2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(&
                    &-6*Hdot**2*(phip1 - 4*a*phip2) + phip1*Hddd) +3*self%QuarticGalileon_c5*phip1*(11*phip1*Hdotdot**2 &
                    &-24*Hdot**3*(2*phip1 - 7*a*phip2) +13*Hdot*phip1*Hddd)) +3*adotoa**4*self%QuarticGalileon_c5*phip1**2*Hdotdot*(11*phip1**2 &
                    &+ 132*a**2*phip2**2 +3*a*phip1*(-21*phip2 + 11*a*phip3)) +adotoa**3*phip1*(-4*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(phip1**2&
                    & + 18*a**2*phip2**2 +6*a*phip1*(-phip2 + a*phip3)) +3*self%QuarticGalileon_c5*phip1*(3*phip1*(-2*phip1 + 7*a*phip2)*Hddd &
                    &+ 4*Hdot**2*(11*phip1**2 + 132*a**2*phip2**2 +3*a*phip1*(-21*phip2 + 11*a*phip3)))) &
                    &+3*adotoa**2*phip1**2*(2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdotdot*(phip1 - 4*a*phip2) &
                    &+self%QuarticGalileon_c5*phip1*(-33*Hdot*Hdotdot*(2*phip1 - 7*a*phip2) + phip1*Hdddd)) &
                    &-adotoa**5*(2*a**4*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(6*phip2**3 + 9*phip1*phip2*phip3 +phip1**2*phip4) &
                    &+3*self%QuarticGalileon_c5*Hdot*phip1*(6*phip1**3 - 276*a**3*phip2**3 +12*a**2*phip1*phip2*(11*phip2 - 23*a*phip3) +a*phip1**2*(-42*phip2 &
                    &+ 33*a*phip3 - 23*a**2*phip4))) +3*a**4*adotoa**7*self%QuarticGalileon_c5*(24*phip2**4 +72*phip1*phip2**2*phip3 &
                    &+16*phip1**2*phip2*phip4 +phip1**2*(12*phip3**2 + phip1*phip5)) ))/(a**4*adotoa*eft_par_cache%h0_Mpc**6*m0**5))
                    !
          self%EFTc%y(ind)         = (adotoa**2*phip1**2*(a**3*self%QuarticGalileon_c2*eft_par_cache%h0_Mpc**6*m0**3 &
                    &+2*Hdot*phip1*(-(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2) +3*Hdot*phip1*(&
                    &-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) + 6*self%QuarticGalileon_c5*Hdot*phip1)) &
                    &+2*adotoa*phip1**2*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) &
                    &+18*self%QuarticGalileon_c5*Hdot*phip1)*Hdotdot -12*adotoa**3*self%QuarticGalileon_c5*phip1**2*Hdotdot*(phip1 &
                    &- 4*a*phip2) +adotoa**2*(-2*a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*(-3*phip1 + a*phip2) &
                    &-3*phip1*(2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(2*phip1 + 3*a*phip2) &
                    &+self%QuarticGalileon_c5*phip1*(15*Hdot**2*(phip1 - 5*a*phip2) -phip1*Hddd))) &
                    &-adotoa**4*(3*self%QuarticGalileon_c5*Hdot*phip1*(6*phip1**2 - 72*a**2*phip2**2 +a*phip1*(25*phip2 &
                    &- 18*a*phip3)) +2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(-11*phip1**2 + 3*a**2*phip2**2 +a*phip1*(5*phip2 &
                    &+ a*phip3))) +3*adotoa**6*self%QuarticGalileon_c5*(7*phip1**3 + 12*a**3*phip2**3 +4*a**2*phip1*phip2*(-phip2 &
                    &+ 3*a*phip3) +a*phip1**2*(-7*phip2 - a*phip3 + a**2*phip4))))/(2.*a*eft_par_cache%h0_Mpc**6*m0**5)

          self%EFTLambda%y(ind)    = (adotoa**2*phip1**2*(a**3*self%QuarticGalileon_c2*eft_par_cache%h0_Mpc**6*m0**3 &
                    &+2*(6*Hdot**2*phip1**2*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) &
                    &+ 6*self%QuarticGalileon_c5*Hdot*phip1) +2*adotoa*phip1**2*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) &
                    &+18*self%QuarticGalileon_c5*Hdot*phip1)*Hdotdot -3*adotoa**3*self%QuarticGalileon_c5*phip1**2*Hdotdot*(phip1 &
                    &- 16*a*phip2) -2*a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*(Hdot*phip1 + a*adotoa**2*phip2) &
                    &+3*adotoa**2*phip1*(-6*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(phip1 + a*phip2) &
                    &+self%QuarticGalileon_c5*phip1*(-3*Hdot**2*(phip1 - 25*a*phip2) +phip1*Hddd)) -2*adotoa**4*(&
                    &a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(phip1**2 + 3*a**2*phip2**2 +a*phip1*(8*phip2 + a*phip3)) &
                    &-3*self%QuarticGalileon_c5*Hdot*phip1*(-3*phip1**2 + 36*a**2*phip2**2 +a*phip1*(4*phip2 + 9*a*phip3))) &
                    &+3*a*adotoa**6*self%QuarticGalileon_c5*(12*a**2*phip2**3 +4*a*phip1*phip2*(2*phip2 + 3*a*phip3) +phip1**2*(&
                    &-4*phip2 + 2*a*phip3 + a**2*phip4)))))/(2.*a*eft_par_cache%h0_Mpc**6*m0**5)

          self%EFTc%yp(ind)      = -(adotoa*phip1*(4*Hdot**2*phip1**2*(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2 &
                    &+3*Hdot*phip1*(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 - 6*self%QuarticGalileon_c5*Hdot*phip1)) &
                    &+2*adotoa*phip1**2*(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2 +9*Hdot*phip1*(&
                    &a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 - 12*self%QuarticGalileon_c5*Hdot*phip1))*Hdotdot&
                    & - 2*a**3*self%QuarticGalileon_c2*eft_par_cache%h0_Mpc**6*m0**3*(Hdot*phip1 + a*adotoa**2*phip2) &
                    &+2*adotoa**2*phip1*(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*Hdot*(-13*phip1 &
                    &+ 7*a*phip2) +phip1*(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(6*Hdot**2*(3*phip1 + 8*a*phip2) &
                    &+ phip1*Hddd)- 6*self%QuarticGalileon_c5*phip1*(3*phip1*Hdotdot**2 +Hdot**3*(-24*phip1 &
                    &+ 90*a*phip2) +4*Hdot*phip1*Hddd))) -3*adotoa**5*self%QuarticGalileon_c5*phip1**2*Hdotdot*(6*phip1**2 &
                    &+ 136*a**2*phip2**2 +a*phip1*(-77*phip2 + 34*a*phip3)) +adotoa**4*(2*a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*(3*phip1**2 &
                    &+ 2*a**2*phip2**2 +a*phip1*(-9*phip2 + a*phip3)) +3*phip1*(2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(&
                    &-26*phip1**2 + 15*a**2*phip2**2 +5*a*phip1*(3*phip2 + a*phip3)) -self%QuarticGalileon_c5*phip1*(-7*phip1*(phip1 - 3*a*phip2)*Hddd &
                    &+ 3*Hdot**2*(3*phip1**2 + 244*a**2*phip2**2 +a*phip1*(-125*phip2 + 61*a*phip3))))) +adotoa**3*phip1**2*(&
                    &2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdotdot*(4*phip1 + 13*a*phip2) +3*self%QuarticGalileon_c5*phip1*(&
                    &2*Hdot*Hdotdot*(43*phip1 - 145*a*phip2) - phip1*Hdddd)) +adotoa**6*(2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(&
                    &22*phip1**3 + 6*a**3*phip2**3 +3*a**2*phip1*phip2*(5*phip2 + 3*a*phip3) +a*phip1**2*(-49*phip2 + 5*a*phip3 + a**2*phip4)) &
                    &-3*self%QuarticGalileon_c5*Hdot*phip1*(74*phip1**3 + 312*a**3*phip2**3 +12*a**2*phip1*phip2*(-17*phip2 &
                    &+ 26*a*phip3) +a*phip1**2*(-36*phip2 - 51*a*phip3 + 26*a**2*phip4))) +3*adotoa**8*self%QuarticGalileon_c5*(&
          ...
          c3= -1.0_dl*self%QuarticGalileon_c3/(eft_par_cache%h0_Mpc)**2

          self%EFTLambda%yp(ind) = self%QuarticGalileon_c2*a2*( adotoa*Hdot*phip1**2 +a*adotoa**3*phip1*phip2 ) &
                    ...
                    &+3._dl*Hdot*phip1*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) + 6._dl*self%QuarticGalileon_c5*Hdot*phip1)) &
                    &-2._dl*adotoa*phip1**2*( +9._dl*Hdot*phip1*(&
                    &a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 - 12._dl*self%QuarticGalileon_c5*Hdot*phip1))*Hdotdot &
                    &-2._dl*adotoa**2*phip1*( &
                    &+phip1*(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(6._dl*Hdot**2*(5._dl*phip1 + 8._dl*a*phip2) + phip1*Hddd)&
                    &-6._dl*self%QuarticGalileon_c5*phip1*(3._dl*phip1*Hdotdot**2 +Hdot**3*(-12._dl*phip1 +90._dl*a*phip2) &
                    &+4._dl*Hdot*phip1*Hddd))) +3._dl*adotoa**5*self%QuarticGalileon_c5*phip1**2*Hdotdot*(-3._dl*phip1**2 +136._dl*a**2*phip2**2 &
                    &+a*phip1*(-29._dl*phip2 +34._dl*a*phip3)) -adotoa**4*(&
                    & +6._dl*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*phip1*(-4._dl*phip1**2 +15._dl*a**2*phip2**2 &
                    &+5._dl*a*phip1*(5._dl*phip2 + a*phip3)) +3._dl*self%QuarticGalileon_c5*phip1**2*(phip1*(4._dl*phip1 -21._dl*a*phip2)*Hddd &
                    &+3._dl*Hdot**2*(9._dl*phip1**2 -244._dl*a**2*phip2**2 +a*phip1*(39._dl*phip2 -61._dl*a*phip3)))) +adotoa**3*phip1**2*(&
                    &-2._dl*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdotdot*(7._dl*phip1 +13._dl*a*phip2) +3._dl*self%QuarticGalileon_c5*phip1*(&
                    &Hdot*Hdotdot*(-47._dl*phip1 +290._dl*a*phip2) + phip1*Hdddd)) -2._dl*adotoa**6*(&
                    &a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(-2._dl*phip1**3 +6._dl*a**3*phip2**3 +3._dl*a**2*phip1*phip2*(8._dl*phip2 +3._dl*a*phip3) &
                    &+a*phip1**2*(-4._dl*phip2 +8._dl*a*phip3 + a**2*phip4)) -3._dl*self%QuarticGalileon_c5*Hdot*phip1*(9._dl*phip1**3 +156._dl*a**3*phip2**3 &
                    &+12._dl*a**2*phip1*phip2*(phip2 +13._dl*a*phip3) +a*phip1**2*(-39._dl*phip2 +3._dl*a*phip3 +13._dl*a**2*phip4))) +3._dl*a*adotoa**8*self%QuarticGalileon_c5*(&
          ...
          self%EFTgamma1%y(ind)  = (adotoa**2*phip1**2*(2*Hdot*phip1*(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2 &
                        &+3*Hdot*phip1*(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 - 6*self%QuarticGalileon_c5*Hdot*phip1)) &
                        &+2*adotoa*phip1**2*(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 -18*self%QuarticGalileon_c5*Hdot*phip1)*&
                        &Hdotdot +12*adotoa**3*self%QuarticGalileon_c5*phip1**2*Hdotdot*(phip1 - 4*a*phip2) &
                        &+adotoa**2*(2*a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*(3*phip1 + a*phip2) +3*phip1*(&
                        &2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(2*phip1 + 3*a*phip2) +self%QuarticGalileon_c5*phip1*(&
                        &15*Hdot**2*(phip1 - 5*a*phip2) -phip1*Hddd))) +adotoa**4*(3*self%QuarticGalileon_c5*Hdot*phip1*(6*phip1**2 &
                        &- 72*a**2*phip2**2 +a*phip1*(25*phip2 - 18*a*phip3)) +2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(16*phip1**2 &
                        &+ 3*a**2*phip2**2 +a*phip1*(5*phip2 + a*phip3))) +3*adotoa**6*self%QuarticGalileon_c5*(13*phip1**3 - 12*a**3*phip2**3 &
                        &+4*a**2*phip1*phip2*(phip2 - 3*a*phip3) +a*phip1**2*(7*phip2 + a*(phip3 - a*phip4)))))/(4.*a**3*eft_par_cache%h0_Mpc**8*m0**5)
                        !
          self%EFTgamma1%yp(ind)  = (phip1*(4*Hdot**2*phip1**2*(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2 &
                        &+3*Hdot*phip1*(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 - 6*self%QuarticGalileon_c5*Hdot*phip1)) &
                        &+2*adotoa*phip1**2*(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2 +9*Hdot*phip1*(&
                        &a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 - 12*self%QuarticGalileon_c5*Hdot*phip1))*Hdotdot &
                        &+ 2*adotoa**2*phip1*(a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*Hdot*(11*phip1 &
                        &+ 7*a*phip2) +phip1*(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(6*Hdot**2*(3*phip1 + 8*a*phip2) &
                        &+ phip1*Hddd)- 6*self%QuarticGalileon_c5*phip1*(3*phip1*Hdotdot**2 +Hdot**3*(-24*phip1 &
                        &+ 90*a*phip2) +4*Hdot*phip1*Hddd))) -3*adotoa**5*self%QuarticGalileon_c5*phip1**2*Hdotdot*(&
                        &6*phip1**2 + 136*a**2*phip2**2 +a*phip1*(-77*phip2 + 34*a*phip3)) +adotoa**4*(2*a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*(&
                        &-3*phip1**2 + 2*a**2*phip2**2 +a*phip1*(9*phip2 + a*phip3)) +3*phip1*(2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(&
                        &28*phip1**2 + 15*a**2*phip2**2 +5*a*phip1*(3*phip2 + a*phip3)) -self%QuarticGalileon_c5*phip1*(-7*phip1*(phip1 &
                        &- 3*a*phip2)*Hddd + 3*Hdot**2*(3*phip1**2 + 244*a**2*phip2**2 +a*phip1*(-125*phip2 + 61*a*phip3))))) &
                        &+adotoa**3*phip1**2*(2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdotdot*(4*phip1 &
                        &+ 13*a*phip2) +3*self%QuarticGalileon_c5*phip1*(2*Hdot*Hdotdot*(43*phip1 - 145*a*phip2) &
                        &- phip1*Hdddd)) +adotoa**6*(3*self%QuarticGalileon_c5*Hdot*phip1*(86*phip1**3 - 312*a**3*phip2**3 &
                        &+12*a**2*phip1*phip2*(17*phip2 - 26*a*phip3) +a*phip1**2*(36*phip2 + 51*a*phip3 - 26*a**2*phip4)) &
                        &+2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(-32*phip1**3 + 6*a**3*phip2**3 +3*a**2*phip1*phip2*(5*phip2 &
                        &+ 3*a*phip3) +a*phip1**2*(59*phip2 + 5*a*phip3 + a**2*phip4))) -3*adotoa**8*self%QuarticGalileon_c5*(39*phip1**4 &
          ...
          self%EFTgamma2%y(ind)  = (adotoa**3*phip1**3*(-2._dl*a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2 &
                        &+2._dl*Hdot*phip1*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) +6._dl*self%QuarticGalileon_c5*Hdot*phip1) &
                        &+3._dl*adotoa*self%QuarticGalileon_c5*phip1**2*Hdotdot -adotoa**2*(3._dl*self%QuarticGalileon_c5*Hdot*phip1*(phip1 &
                        &-11._dl*a*phip2) +2._dl*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(6._dl*phip1 + a*phip2)) +3._dl*adotoa**4*self%QuarticGalileon_c5*(&
                        &-5._dl*phip1**2 +4._dl*a**2*phip2**2 +a**2*phip1*phip3)))/(a**2*eft_par_cache%h0_Mpc**7*m0**5)

          self%EFTgamma2%yp(ind)  = (adotoa*phip1**2*(6._dl*Hdot**2*phip1**2*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) &
                        &+ 6._dl*self%QuarticGalileon_c5*Hdot*phip1) +2._dl*adotoa*phip1**2*(-(a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0) &
                        &+18._dl*self%QuarticGalileon_c5*Hdot*phip1)*Hdotdot +3._dl*adotoa**3*self%QuarticGalileon_c5*phip1**2*Hdotdot*(-3._dl*phip1 &
                        &+ 16._dl*a*phip2) -6._dl*a**2*self%QuarticGalileon_c3*eft_par_cache%h0_Mpc**4*m0**2*(Hdot*phip1 + a*adotoa**2*phip2) &
                        &+adotoa**2*phip1*(-2._dl*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*Hdot*(29._dl*phip1 + 9._dl*a*phip2) &
                        &+3._dl*self%QuarticGalileon_c5*phip1*(Hdot**2*(-13._dl*phip1 + 75._dl*a*phip2) +phip1*Hddd)) &
                        &-adotoa**4*(2._dl*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(-6._dl*phip1**2 + 3._dl*a**2*phip2**2 +a*phip1*(24._dl*phip2 &
                        &+ a*phip3)) +3._dl*self%QuarticGalileon_c5*Hdot*phip1*(33._dl*phip1**2 - 72._dl*a**2*phip2**2 -2._dl*a*phip1*(-8._dl*phip2 + 9._dl*a*phip3))) &
                        &+3._dl*adotoa**6*self%QuarticGalileon_c5*(10._dl*phip1**3 +12._dl*a**3*phip2**3 +12._dl*a**3*phip1*phip2*phip3 +phip1**2*(-25._dl*a*phip2 &
                        &+ a**3*phip4))))/(a**3*eft_par_cache%h0_Mpc**7*m0**5)

          self%EFTgamma3%y(ind)  = (adotoa**4*phip1**4*(-2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 &
                        &+3*self%QuarticGalileon_c5*(Hdot*phip1 + adotoa**2*(-phip1 + a*phip2))))/(a*eft_par_cache%h0_Mpc**6*m0**5)

          self%EFTgamma3%yp(ind)  = (adotoa**2*phip1**3*(-8*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(Hdot*phip1 &
                        &+ a*adotoa**2*phip2) +3*self%QuarticGalileon_c5*(4*Hdot**2*phip1**2 + adotoa*phip1**2*Hdotdot &
          ...
          self%EFTgamma4%ypp(ind) =-(phip1**2*(8*Hdot**2*phip1**2*(-2*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 &
                        &+ 3*self%QuarticGalileon_c5*Hdot*phip1) +adotoa*phip1**2*(-8*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0 &
                        &+33*self%QuarticGalileon_c5*Hdot*phip1)*Hdotdot +3*adotoa**3*self%QuarticGalileon_c5*phip1**2*&
                        &Hdotdot*(-9*phip1 + 16*a*phip2) +adotoa**2*phip1*(8*a*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*&
                        &m0*Hdot*(phip1 - 8*a*phip2) +3*self%QuarticGalileon_c5*phip1*(Hdot**2*(-36*phip1 + 64*a*phip2) &
                        &+phip1*Hddd)) +adotoa**4*(-8*a**3*self%QuarticGalileon_c4*eft_par_cache%h0_Mpc**2*m0*(3*phip2**2 &
                        &+ phip1*phip3) +3*self%QuarticGalileon_c5*Hdot*phip1*(20*phip1**2 + 68*a**2*phip2**2 +a*phip1*(-76*phip2 &
                        &+ 17*a*phip3))) +3*adotoa**6*self%QuarticGalileon_c5*(-2*phip1**3 + 12*a**3*phip2**3 +4*a**2*phip1*phip2*(&
                        &-5*phip2 + 3*a*phip3) +a*phip1**2*(10*phip2 - 5*a*phip3 + a**2*phip4))))/(a**3*eft_par_cache%h0_Mpc**6*m0**5)

          end subroutine

    end subroutine EFTCAMBQuarticGalileonSolveBackground

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that computes the number of parameters of the model.
    subroutine EFTCAMBQuarticGalileonComputeParametersNumber( self )
    ...
    end subroutine EFTCAMBQuarticGalileonComputeParametersNumber

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that prints on the screen feedback information about the model.
    subroutine EFTCAMBQuarticGalileonFeedback( self, print_params )
        ...
        if ( self%QuarticGalileon_c2 == -1._dl ) then

          write(*,*)
          write(*,'(a,a)')    '   Model               =  ', self%name
          write(*,'(a,I3)')   '   Number of params    ='  , self%parameter_number
          write(*,'(a,F12.6)')   '                 xi    ='  , self%csi
          write(*,'(a,F12.6)')   '                 c2    ='  , self%QuarticGalileon_c2
          write(*,'(a,F12.6)')   '                 c3    ='  , self%QuarticGalileon_c3
          write(*,'(a,F12.6)')   '                 c4    ='  , self%QuarticGalileon_c4
          write(*,'(a,F12.6)')   '                 c5    ='  , self%QuarticGalileon_c5
    ...
    end subroutine EFTCAMBQuarticGalileonFeedback

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that returns the i-th parameter name of the model
    subroutine EFTCAMBQuarticGalileonParameterNames( self, i, name )
    ...
    end subroutine EFTCAMBQuarticGalileonParameterNames

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that returns the i-th parameter name of the model
    subroutine EFTCAMBQuarticGalileonParameterNamesLatex( self, i, latexname )
    ...
    end subroutine EFTCAMBQuarticGalileonParameterNamesLatex

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that returns the i-th parameter name of the model
    subroutine EFTCAMBQuarticGalileonParameterValues( self, i, value )
    ...
    end subroutine EFTCAMBQuarticGalileonParameterValues

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that computes the value of the background EFT functions at a given time.
    subroutine EFTCAMBQuarticGalileonBackgroundEFTFunctions( self, a, eft_par_cache, eft_cache )
    ...
    subroutine EFTCAMBQuarticGalileonSecondOrderEFTFunctions( self, a, eft_par_cache, eft_cache )
    ...
    subroutine EFTCAMBQuarticGalileonComputeAdotoa( self, a, eft_par_cache, eft_cache )
    ...
    end subroutine EFTCAMBQuarticGalileonComputeAdotoa

    ! ---------------------------------------------------------------------------------------------
    !> Subroutine that computes the two derivatives wrt conformal time of H.
    subroutine EFTCAMBQuarticGalileonComputeHubbleDer( self, a, eft_par_cache, eft_cache )
    ...
    end subroutine EFTCAMBQuarticGalileonComputeHubbleDer

    ! ---------------------------------------------------------------------------------------------
    !> Function that computes model specific stability requirements.
    function EFTCAMBQuarticGalileonAdditionalModelStability( self, a, eft_par_cache, eft_cache )
...
end module EFTCAMB_full_Quartic_Galileon
