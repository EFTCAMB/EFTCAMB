...
class CAMB_results_plot:
    ...
    def __init__(self, root, outpath, name=''):
        """
        Class constructor:
        ...
        self.scalarCovCls        = np.loadtxt(root+'_scalarCovCls.dat')

        if os.path.exists(root+'_lensedCls.dat'):
            self.lensing = True
        ...
        if os.path.exists(root+'_matterpower.dat'):
            self.transfer = True
        ...
        if os.path.exists(root+'_tensCls.dat'):
            self.tensor = True
    ...
    def plot_scalCls(self):
        ...
        plt.savefig(self.outpath+self.name+'_scalCls.pdf')
        plt.clf()


    def plot_scalarCovCls(self):
        """
        Plots and saves all the scalar covariance Cls in a unique image
        """
        # number of Cls:
        num     = int(math.sqrt(self.scalarCovCls.shape[1]-1))

        # l values:
        xvalues = self.scalarCovCls[:,0]
        # set up the plots:
        plots       = CMB_plots()
        plots.color = self.color
        ...
        fig.subplots_adjust(top=0.90) # adjust for the fact that tight_layout does not consider suptitle
        # save the figure and close
        plt.savefig(self.outpath+self.name+'_scalarCovCls.pdf')
        plt.clf()
    ...
    def plot_lensedCls(self):
        ...
        fig.subplots_adjust(top=0.90) # adjust for the fact that tight_layout does not consider suptitle
        # save the figure and close
        plt.savefig(self.outpath+self.name+'_lensCls.pdf')
        plt.clf()
    ...
    def plot_tensCls(self):
        ...
        # save the figure and close
        plt.savefig(self.outpath+self.name+'_tensCls.pdf')
        plt.clf()
    ...
    def plot_totalCls(self):
        ...
        fig.subplots_adjust(top=0.90) # adjust for the fact that tight_layout does not consider suptitle
        # save the figure and close
        plt.savefig(self.outpath+self.name+'_totCls.pdf')
        plt.clf()
    ...
    def plot_Transfer(self):
        ...
        # save the figure and close
        plt.savefig(self.outpath+self.name+'_transfer.pdf')
        plt.clf()
