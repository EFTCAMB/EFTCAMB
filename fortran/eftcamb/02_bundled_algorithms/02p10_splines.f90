module splines
    ...
    ! This file is taken from https://github.com/certik/fortran-utils
...
contains
    ...
    subroutine spline3ders(x, y, xnew, ynew, dynew, d2ynew)
        ...
        real(dl), intent(in) :: x(:), y(:), xnew(:)
        real(dl), intent(out), optional :: ynew(:), dynew(:), d2ynew(:)
        real(dl) :: c(0:4, size(x)-1)
        integer :: i, ip
        call spline3pars(x, y, [2, 2], [0._dl, 0._dl], c)
    ...
    subroutine spline3pars(xi,yi,bctype,bcval,c)
        ...
        real(dl), intent(in):: bcval(2)     ! boundary condition values at each end:
        ...
        real(dl) As(5,2*size(c,2))             ! spline eq. matrix -- LAPACK band form
        real(dl) bs(2*size(c,2))               ! spline eq. rhs vector
        real(dl) cs(2*size(c,2))               ! spline eq. solution vector
        real(dl) hi(size(c,2))                 ! spline intervals
        real(dl) Ae(4,4)                       ! end-cubic eq. matrix
        real(dl) be(4)                         ! end-cubic eq. rhs vector
        real(dl) ce(4)                         ! end-cubic eq. solution vector
        real(dl) xe(4),ye(4)                   ! x,y values at ends
        real(dl) d2p1,d2pn                     ! 2nd derivatives at ends
        real(dl) x0                            ! expansion center
        real(dl) c1,c2,c3,c4                   ! expansion coefficients
        integer n                              ! number of data points
        integer i,j,i2
        ! lapack variables
        integer ipiv(4),ipiv2(2*size(c,2))
        real(dl) bemat(4,1),bmat(2*size(c,2),1)
        integer info
        ...
        !if (info /= 0) call stop("spline3pars error: dgesv error.")
        !cs=bmat(:,1)
