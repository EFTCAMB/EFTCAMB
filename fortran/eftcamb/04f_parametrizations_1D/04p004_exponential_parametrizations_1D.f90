...
contains
    ...
    function ExponentialParametrized1DFourthDerivative( self, x, eft_cache )
        ...
        ExponentialParametrized1DFourthDerivative = self%coefficient*self%exponent*(x**(self%exponent-4._dl))&
        & *Exp(self%coefficient*x**self%exponent)*(-6._dl*self%exponent**2*(self%coefficient**2*x**(2._dl*self%exponent) &
        & + 3._dl*self%coefficient*x**self%exponent + 1._dl) + self%exponent**3 * (self%coefficient**3 * x**(3._dl*self%exponent) &
        & + 6._dl*self%coefficient**2 * x**(2._dl*self%exponent) + 7._dl*self%coefficient*x**(self%exponent)+ 1._dl) &
        & + 11._dl*self%exponent*(self%coefficient*x**(self%exponent) + 1._dl) - 6._dl)
