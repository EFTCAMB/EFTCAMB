...
module EFTCAMB_cosine_parametrizations_1D
    ...
    contains
        ...
        procedure :: fourth_derivative     => Cosine1DFourthDerivative    !< function that returns the fourth derivative of the Cosine.
...
contains
    ...
    function Cosine1DFourthDerivative( self, x, eft_cache )
        ...
        real(dl), intent(in)                               :: x         !< the input scale factor
        type(TEFTCAMB_timestep_cache), intent(in), optional :: eft_cache !< the optional input EFTCAMB cache
        real(dl) :: Cosine1DFourthDerivative                            !< the output value

        Cosine1DFourthDerivative = self%alpha**4*self%f0*Cos( self%beta + self%alpha*x )

    end function Cosine1DFourthDerivative
