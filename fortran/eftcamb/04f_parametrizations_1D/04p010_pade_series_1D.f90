...
module EFTCAMB_pade_series_1D
    ...
    contains
        ...
        procedure :: fourth_derivative     => PadeSeriesParametrized1DFourthDerivative    !< function that returns the fourth derivative of the Pade expansion.
...
contains
    ...
    subroutine PadeSeriesParametrized1DInitFunctionFromFile( self, Ini, eft_error )
    ...
    function PadeSeriesParametrized1DFourthDerivative( self, x, eft_cache )
        ...
        real(dl), intent(in)                               :: x         !< the input scale factor
        type(TEFTCAMB_timestep_cache), intent(in), optional :: eft_cache !< the optional input EFTCAMB cache
        real(dl) :: PadeSeriesParametrized1DFourthDerivative           !< the output value

        real(dl) :: x_temp, up, dup, ddup, dddup, down, ddown, dddown, ddddown, ddddup, dddddown
        integer  :: ind
        ...
        ddup    = self%taylor_up%second_derivative( x, eft_cache )
        dddup   = self%taylor_up%third_derivative( x, eft_cache )
        ddddup  = self%taylor_up%fourth_derivative( x, eft_cache )
        down    = self%taylor_down%value( x, eft_cache )
        ddown   = self%taylor_down%first_derivative( x, eft_cache )
        dddown  = self%taylor_down%second_derivative( x, eft_cache )
        ddddown = self%taylor_down%third_derivative( x, eft_cache )
        dddddown = self%taylor_down%fourth_derivative( x, eft_cache )

        PadeSeriesParametrized1DFourthDerivative = (ddddup/down) - 4._dl*(dddup*ddown/down**2) + 6._dl*ddup*(2._dl*ddown**2/down**3 &
        & -dddown/down**2) + 4._dl*(-ddddown/down**2 - 6._dl*ddown**3/down**4 + 6._dl*ddown*dddown/down**3) &
        & +up*(-dddddown/down**2 + 6._dl*dddown**2/down**3 + 24._dl*ddown**4/down**5 + 8._dl*ddddown*ddown/down**3 &
        & -36._dl*dddown*ddown**2/down**4)

    end function PadeSeriesParametrized1DFourthDerivative
